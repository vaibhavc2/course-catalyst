...
const httpCheck = asyncFnWrapper(async (url: string): Promise<CheckResult> => {
...
const dbCheck = asyncFnWrapper(async (): Promise<CheckResult> => {
  try {
...
const diskCheck = asyncFnWrapper(async (): Promise<CheckResult> => {
  try {
...
const memoryCheck = asyncFnWrapper(async (): Promise<CheckResult> => {
  try {
    ...
    const usedPercentage = (parseInt(usedMemory) / parseInt(totalMemory)) * 100;
